...
predicted_mask_name = config['predictedmask']
gt_mask_name = config['gt_mask']
rect_fill_color = config.get('fillcolor', None)
which_set = config.get('which_set', 'test')


def overlay_mask(image, mask, gt_mask, mask_color_pred=(0, 255, 255, 128), mask_color_gt=(0, 128, 255, 255)):
    ...
    mask_colored1 = Image.new("RGBA", image.size, mask_color_pred1)
    mask_colored1.putalpha(mask1)  # Apply the first mask to the first colored image

    mask_colored2 = Image.new("RGBA", image.size, mask_color_pred2)
    mask_colored2.putalpha(mask2)  # Apply the second mask to the second colored image
    ...
    final_image = Image.alpha_composite(image.convert("RGBA"), combined_mask)
...
class ImageToggleApp:
    def __init__(self, root):
        ...
        self.mask_name_label = tk.Label(self.button_frame)
        self.mask_name_label.pack(side=tk.LEFT, padx=(85, 20))
        self.mask_name_label.config(text=f"Mask Name: {predicted_mask_name}")    
    ...
    def upload_images(self):
        ...
        gt_mask_path = os.path.join(self.label.cget("text"), gt_mask_name)
        if os.path.exists(change_mask_path):        
            self.change_mask = Image.open(change_mask_path)
            self.gt_mask = Image.open(gt_mask_path)
        else:
